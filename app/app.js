...
(function () {
  ...
  function renderQuiz() {
    ...
    '<div style="display:flex;flex-direction:column;min-height:100vh;">' +
      ...
      '<div style="padding:12px 18px 22px;position:sticky;bottom:0;background:linear-gradient(to top,#F7F8FB 72%,transparent);display:flex;gap:10px;justify-content:space-between;align-items:center;">' +
        prevBtn +
        '<button data-action="nextPart" class="a-scale99" style="flex:0 0 auto;border:none;background:#1A1D24;color:#fff;font-size:14px;font-weight:700;padding:11px 30px;border-radius:11px;cursor:pointer;font-family:inherit;">' + (isLastPart ? "결과 보기" : "다음 파트 →") + '</button>' +
      '</div>' +
    '</div>';
  }
